...
def apply(table):
    ...
    if(tempbuff):
        ...
        query_apply_job = f'INSERT INTO mydb.application VALUES(NULL, (SELECT rid FROM mydb.job WHERE job.jid={ajid}), {ajid}, {clicid})'
        mycon = sql.connect(host='localhost', user='root', passwd=user_pwd, database='mydb')
        cur = mycon.cursor()

        try:
            cur.execute(query_apply_job)
            mycon.commit()
        ...
        # Check the number of rows affected
            if cur.rowcount > 0:
                ...
                messagebox.showinfo('Not Applicable', 'Your application is not applicable')
        except sql.Error as e:
            # Check if the error message indicates a qualification check violation
            if "application_check_qualification" in str(e):
                messagebox.showinfo('Oops', 'You do not have the required qualification for this job')
            else:
                messagebox.showinfo('Oops','You do not have the required qualification for this job')
